...
public class CarritoService {
	...
	public CarritoInscripcion obtenerOCrearCarrito(Long usuarioId) {
		return carritoRepo.findByUsuarioIdAndEstado(usuarioId, "PENDIENTE").orElseGet(() -> {
			CarritoInscripcion nuevo = CarritoInscripcion.builder().usuarioId(usuarioId).estado("PENDIENTE")
					.fechaCreacion(LocalDateTime.now()).totalCreditos(0).montoEstimado(0.0).detalles(new ArrayList<>()) // 🔥
																														// inicializa
																														// lista
																														// vacía
					.build();
	...
	public void confirmarPago(Long usuarioId) {
		...
		// Marcar carritoInscripcion como PAGADO
		carrito.setEstado("PAGADO");
		carrito.setFechaPago(LocalDateTime.now());
		carritoRepo.save(carrito);

		// 🔥 Guardar historial en tabla "carrito"
		Carrito historial = Carrito.builder().usuarioId(carrito.getUsuarioId()).estado(carrito.getEstado())
				.fechaCreacion(carrito.getFechaCreacion()).fechaPago(carrito.getFechaPago())
				.montoEstimado(carrito.getMontoEstimado()).totalCreditos(carrito.getTotalCreditos()).build();

		carritoRepository.save(historial);
	}

	/**
	 * Obtener el último carrito pagado por usuario
	 */
	public Carrito obtenerUltimoCarritoPagadoPorUsuarioId(Long usuarioId) {
		return carritoRepository
				.findTopByUsuarioIdAndEstadoIgnoreCaseAndFechaPagoIsNotNullOrderByFechaPagoDesc(usuarioId,
						"PAGADO".trim().toUpperCase())
				.orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND,
						"El usuario no tiene un carrito pagado"));
