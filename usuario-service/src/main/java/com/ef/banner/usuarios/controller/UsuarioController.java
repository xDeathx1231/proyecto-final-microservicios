...
public class UsuarioController {
	...
	public ResponseEntity<BoletaPagoDTO> generarBoleta(@PathVariable Long usuarioId, @RequestParam Long carritoId) {
		...
		CarritoDTO carrito = carritoClient.obtenerCarritoPagadoPorUsuarioId(usuarioId);
	...
	public ResponseEntity<List<UsuarioDTO>> listarPorRol(@PathVariable String rol) {
		Usuario.Rol rolEnum;
		try {
			rolEnum = Usuario.Rol.valueOf(rol.toUpperCase());
		} catch (IllegalArgumentException e) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Rol inválido: " + rol);
		}
		return ResponseEntity.ok(service.listarPorRol(rolEnum));
	}
}
